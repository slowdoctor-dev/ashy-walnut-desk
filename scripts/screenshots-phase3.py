#!/usr/bin/env python3
"""Capture Phase 3 AuditLive + Compensation screenshots."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--inbox-id", required=True)
    p.add_argument("--out", default="docs/phase-3-screenshots")
...
def main() -> int:
    ...
    with sync_playwright() as pw:
        ...
        try:
            ...
            # 1. Audit chain — topic list
            page.goto(urljoin(args.url, "/audit/chain"))
            page.wait_for_selector("[data-role='topic-list']", state="visible", timeout=10_000)
            shot(page, out_dir, "01-audit-chain-topics.png")

            # 2. Audit chain — events for the seeded inbox
            page.goto(urljoin(args.url, f"/audit/chain?topic={args.inbox_id}"))
            ...
            shot(page, out_dir, "02-audit-chain-events.png")

            # 3. InboxLive.Show — compensation trigger affordance
            page.goto(urljoin(args.url, f"/inbox/{args.inbox_id}"))
            page.wait_for_selector("[data-role='compensation-section']", state="visible", timeout=10_000)
            shot(page, out_dir, "03-inbox-executed-trigger-compensation.png")
