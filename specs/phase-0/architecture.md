...
- **Resource migrations** are generated by `mix ash_postgres.generate_migrations --name <desc>`
  per AGENTS.md §5. `mix ecto.gen.migration` is forbidden (AGENTS.md §9 NEVER list).
- **Setup migrations that pre-date Ash resources** (e.g., the extension-enable
  migration in step 1 below) are authored as **plain Ecto migration files written
  directly** under `priv/repo/migrations/<ts>_<name>.exs`. The file is hand-created;
  the `mix ecto.gen.migration` generator is still not used. This narrow exemption
  is the only path for migrations that have no corresponding Ash resource and must
  exist before any resource migration runs.
...
1. **`<ts>_enable_extensions.exs`** — `CREATE EXTENSION IF NOT EXISTS vector;
   CREATE EXTENSION IF NOT EXISTS pg_trgm;`. Plain Ecto migration file written
   directly per the Authoring exemption above; it runs before any resource
   migration.
...
- AshPaperTrail is installed and configured at the app level (Story 0.4).
- `User` is a sensitive-record resource (carries `email`/`email_hash` marked
  `sensitive? true`) and therefore **must** opt in to AshPaperTrail in Phase 0
  per AGENTS.md §6 ("ALL sensitive-record changes audited via AshPaperTrail").
  Story 0.5 enforces this. If the generated AshAuthentication shape resists a
  clean opt-in, Story 0.5 is blocked pending an ADR amendment — not silently
  deferred.
- `Token` (Phase 0) is also sensitive but uses the AshAuthentication-managed
  token-resource pattern; PaperTrail on `Token` is not a Phase 0 requirement
  (tokens are short-lived and revocation events are captured by their lifecycle).
...
- §7 rule 3 (audit trail mandatory): Auth events route through Ash actions and
  are audited via AshPaperTrail opted in on `User` in Story 0.5 (mandatory —
  see §9 Audit). Migration history is its own audit trail (git + Ecto migration
  versioning).
...
- AshPaperTrail is configured at the app level (Story 0.4) **and** opted in on
  `User` (Story 0.5), satisfying AGENTS.md §6's sensitive-record audit
  requirement for the only sensitive-record resource Phase 0 introduces.
...
| 3. Audit trail mandatory | Framework installed (Story 0.4); `User` opted in (Story 0.5) — covers Phase 0's only sensitive-record resource |
...
Resolved during sections §1–§3 and the Codex review pass:
...
- ✅ AshPaperTrail on `User` → opts in during Phase 0 (Story 0.5),
  mandatory per AGENTS.md §6 (§9 Audit, §10 Audit trail coverage).

Remaining for the PM persona / per-story discussion (none are blockers for
breaking Phase 0 into stories):

1. **Production mailer.** Phase 0 uses Swoosh local mailbox in dev and the
...
2. **Healthcheck endpoint.** No Phase 0 AC requires `/healthz`. Defer to
   Phase 5 unless a CI / dev-loop pressure surfaces earlier. Adding it now
   would be premature.

3. **AshAdmin in Phase 0.** `AGENTS.md §4` lists `ash_admin` in the stack.
...
4. **Telemetry log handler.** Sign-in success/failure are emitted as
